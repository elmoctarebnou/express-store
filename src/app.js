...
app.post('/user', (req, res) => {
  ...
  if(username.length < 6 || username.length > 20){
    return res.status(400).send('Username must be between 6 and 20 character');
  }
  if(password.length < 8 || password.length > 36){
    return res.status(400).send('Password must be between 8 and 36 characters');
  }
  // validate password by making sure it contains at least one digit
  ...
  users.push(newUser);
  res.status(201).location(`http://localhost:${PORT}/user/${id}`).json(newUser);
});
